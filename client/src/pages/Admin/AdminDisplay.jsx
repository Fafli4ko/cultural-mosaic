...
export default function AdminDisplay() {
  ...
  // Function to fetch data based on subpage
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${subpage}/${subpage}`); // Adjust the backend endpoint accordingly
  ...
  return (
    <div className="bg-background min-h-screen ">
      ...
      <div className="mt-8 px-5 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-3">
        {content.length > 0 &&
          content.map((item) => (
            <Link
              ...
              key={item._id}
            >
              <div className="w-full aspect-3/4 bg-mContrastWhite flex items-center justify-center overflow-hidden rounded-lg">
